...
public class BST_1<T extends Comparable<? super T>> {
    class Entry<T> {
        T element;
 ...
 // Return an array with the elements using a level order traversal of the tree
    void levelOrder(Entry<T> node, Comparable[] arr, int index){
    	ArrayDeque<Entry<T>> lst= new ArrayDeque<>();
    	lst.add(node);
    	...
    	while(!lst.isEmpty()){
    		Entry<T> temp=lst.pop();
    		if(temp.left!=null){
    			arr[index++]=temp.left.element;
    			lst.add(temp.left);
    		}
    		if(temp.right!=null){
    			arr[index++]=temp.right.element;
    			lst.add(temp.right);
    		}
    	}
